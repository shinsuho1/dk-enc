...
window.addEventListener("DOMContentLoaded", () => {
    ...
    if (location.href.split('/')[(location.href.split('/').length - 2)] == "main") {
        let perfor_btn = document.querySelectorAll(".perfor_btn");
        let perfor_lis = document.querySelectorAll(".performance .inner>ul>li");
        perfor_btn.forEach((el, index) => {
            el.addEventListener("mouseenter", () => {
                perfor_lis[index].classList.add("hover");
            });
        });
        perfor_lis.forEach((el, index) => {
...
if (form) {
    ...
    button_submit.addEventListener("click", () => {
        ...
        if (!num.test(input_phone.value)) {
            alert("Please enter your company tel.");
            return false;
        }
